...
app.post(['/api/generation/generate', '/api/v1/generation/generate'], auth, async (req, res) => {
  try {
    ...
    // Add subtle variation to prompt to prevent caching
    const varyPrompt = `${finalPrompt} [${seed}]`;

    // Generate image URL based on provider
    let imageUrl;

    if (activeModel.provider === 'Pollinations') {
      imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(varyPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
    } else if (activeModel.provider === 'Replicate') {
      // TODO: Add Replicate API integration
      imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(varyPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
    } else if (activeModel.provider === 'MiniMax') {
      // TODO: Add MiniMax API integration
      imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(varyPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
    } else if (activeModel.provider === 'Stability') {
      // TODO: Add Stability AI integration
      imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(varyPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
    } else {
      // Default to Pollinations with random seed
      imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(varyPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
    }
