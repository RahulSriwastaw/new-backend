...
app.post(['/api/generation/generate', '/api/v1/generation/generate'], auth, async (req, res) => {
  try {
    ...
    try {
      // Use Pollinations as free fallback for now (will generate actual image)
      const pollinationsUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(finalPrompt)}?width=1024&height=1024&nologo=true&seed=${seed}&enhance=true`;
        ...
        const uploadStream = cloudinary.uploader.upload_stream(
          {
            folder: 'generations',
            resource_type: 'image',
            public_id: `gen_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          },
          (error, result) => {
      ...
      // Fallback: Use Pollinations URL directly if upload fails
