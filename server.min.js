...
app.post(['/api/generation/generate', '/api/v1/generation/generate'], auth, async (req, res) => {
  try {
    ...
    if (!activeModel) {
      return res.status(500).json({ error: 'No active AI model configured' });
    }
    ...
    // Get face reference from uploaded images
    let faceReferenceUrl = null;
    if (uploadedImages && uploadedImages.length > 0) {
      // Use first uploaded image as face reference
      faceReferenceUrl = uploadedImages[0];
    }
    ...
    try {
      // Use MiniMax I2I API for generation with face reference
      const MinimaxI2IService = require('./services/minimaxI2IService');
      const minimaxService = new MinimaxI2IService(process.env.MINIMAX_API_KEY || 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..mYBGT5ARHyAWOjJ8vLESvjcH_tYbPHhzVx5MVgZ-z0inrAu4Dj5te3ikdjTf40jxT6S2VpgjQgEL1_5skgpSKxJR_y0jTfvU_TAhWYIjlJZNTh8OxKDfvRdJ4YfIJwDXQiJ7qe11bpVzAAltNHmG-7VghwG6h1d4QHSaTTnlALQcZEAl4Fvp8H-MLD0r5eceR1-kfG9nP6PYDGKJ3l70MzYlGLb2SX9KkDiWYeyfEBKEKZofPoVEE-9IryQIMs5KLcfv-QFxLQvwuRmoBOzuhFz3yaO9jYfZOgKVm4E2E2LeRYzlP83v08p9e3u7YRsy5AvFZOv-HfdO-h6fEn5sE6Q');

      // Call MiniMax I2I with face reference and prompt
      const generationResult = await minimaxService.generateWithCharacter({
        prompt: finalPrompt,
        characterImageUrl: faceReferenceUrl,
        aspectRatio: aspectRatio || '1:1',
        quality: quality || 'HD',
        seed: seed
      });

      // MiniMax returns image URL - download it
      ...
      // Upload to Cloudinary (Account 3 - Generated Images)
      const uploadPromise = new Promise((resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream(
          {
            cloud_name: 'dkeigiajt',
            folder: 'generations',
            resource_type: 'image',
            public_id: `gen_${user._id}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
          },
          (error, result) => {
      ...
      generatedImageUrl = await uploadPromise;
    ...
    await Transaction.create({
      ...
      amount: cost,
      type: 'debit',
      description: `Image generation via ${activeModel.name} (${quality})`,
      gateway: 'System',
      status: 'success',
      date: new Date()
    });
